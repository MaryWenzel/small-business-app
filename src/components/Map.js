...
export default function Map(props) {
  ...
  const globalListing = props.listings.find(
  ...
  useEffect(async () => {
    ...
    if (address) {
      const results = await provider.search({ query: address + ", Dallas" });
      if (results[0]) {
  ...
  if (results) {
    ...
    return (
      <MapContainer
        center={[32.77870502895334, -96.80280346156913]}
        zoom={12}
        scrollWheelZoom={false}
        style={{ height: "500px", width: "500px" }}
      >
